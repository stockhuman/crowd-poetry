...
if __name__ == "__main__":
    ...
    # filepath = "audio_clips/huge_yd923WuUYcE.mp3"
    # start, end = segment(filepath, query)
    # if start is not None:
    #     trimmed_audio = trim(filepath, start, end, F"trim_{filepath}")
    ...
    try:
        ...
        if entries:
            ...
            if start is not None:
                trimmed_audio = trim(filepath, start, end, F"audio_clips/trim_{query}_{video_id}.mp3")
                print(f"Trimmed file saved as: {trimmed_audio}")
            else:
                print("Phrase not found in audio.")
